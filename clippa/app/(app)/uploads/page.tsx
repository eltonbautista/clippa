...
export default function UploadsPage() {
  const { uploads, startProcessing, deleteUpload } = useUploads();

  const uploadsItems = uploads.map(upload => {
    const createdDate = upload.uploadedAt.toISOString().split('T')[0];
    
    return (
      <TableItem key={upload.id} id={upload.id} fileName={upload.title} status={upload.status} created={createdDate} deleteUpload={deleteUpload}>
